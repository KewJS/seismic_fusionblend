...
def compute_alpha(beta, t):
    ...
    a = (1 - beta).cumprod(dim=0).index_slice(0, t + 1).view(-1, 1, 1, 1)
    return a
...
def parse_args_and_config():
    ...
    parser.add_argument("--type", type=str, required=True, help="Either [2d, 3d]")
    ...
    parser.add_argument("--ckpt_load_name", type=str, default="AAPM256_1M.pt", help="Load pre-trained ckpt")
    ...
    parser.add_argument("--lamb", type=float, default=0.04, help="lambda for TV")
    parser.add_argument("--gamma", type=float, default=1.0, help="regularizer for noisy recon")
    parser.add_argument("--T_sampling", type=int, default=50, help="Total number of sampling steps")
    parser.add_argument("--resume_checkpoint", type=bool, default=False, help="resume training from a previous checkpoint")
    
    parser.add_argument("-i", "--image_folder", type=str, default="./results", help="The folder name of samples",)
    parser.add_argument("--dataset_path", type=str, default="/media/harry/tomo/AAPM_data_vol/256_sorted/L067", help="The folder of the dataset")
    parser.add_argument("--mask_type", type=str, default="uniform1d", help="Undersampling type")
    parser.add_argument("--acc_factor", type=int, default=4, help="acceleration factor")
    parser.add_argument("--nspokes", type=int, default=30, help="Number of sampled lines in radial trajectory")
    parser.add_argument("--center_fraction", type=float, default=0.08, help="ACS region")
...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        if self.model_var_type == "fixedlarge":
            ...
            self.logvar = posterior_variance.clamp(min=1e-0).log()
